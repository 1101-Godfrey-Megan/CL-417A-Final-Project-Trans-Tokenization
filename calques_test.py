...
# Calques
def count_calques(preds, refs, spanish):
    total = 0
    details = []
    for pred, ref in zip(preds, refs):
        for token in pred.split():
            if token in spanish and token not in ref.split():
                total += 1
                details.append((token, pred))
    return total, details


tt_total, tt_details = count_calques(tt_preds, references, tokenizer.get_vocab())
v_total, v_details = count_calques(v_preds, references, tokenizer.get_vocab())

print("Trans-tokenization calques:", tt_total)
print("Vanilla mBART calques:", v_total)
