...
### Running:
  Run create_vanilla_ft.py. Then run create_transtokenizer_ft_1.py, after which you should run the following in your terminal:
    ...
    ./fast_align/build/fast_align -i parallel.txt -d -o -v > forward.align
    ./fast_align/build/fast_align - i parallel.txt -d -o -v -r > backward.align
    ./fast_align/build/atools -i forward.align -j backward.align -c grow-diag-final-and > token_relations.txt

After this run create_tokenizer_ft_2.py, generate_predictions.py, then the *test_type*_test.py files (perplexity_test.py, chrF_test.py, repetition_test.py, calques_test.py, grammatical_features_count_test.py). In generate_predictions.py, beam=3 can be changed to beam=1 or beam=9, to create 3 separate csv files, and manually use a hierarchy of 3 > 1 > 9 to recreate the smooth generation mentioned in the study.   
